...
class SINCModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            SINCConv(1 if i == 0 else hidden_dim, hidden_dim, hidden_dim, self.activation) 
...
class GCNModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            dgl.nn.GraphConv(1 if i == 0 else hidden_dim, hidden_dim, allow_zero_in_degree=True)
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class SAGEModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            dgl.nn.SAGEConv(1 if i == 0 else hidden_dim, hidden_dim, 'pool')
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class GATModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, num_heads=1, **kwargs):
        ...
        self.convs = nn.ModuleList([
            dgl.nn.GATv2Conv(1 if i == 0 else hidden_dim, hidden_dim, num_heads, allow_zero_in_degree=True, share_weights=True)
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class GINModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, mlp_layers=2, **kwargs):
        ...
        self.convs = nn.ModuleList([
            dgl.nn.GINConv(MLP(1 if i == 0 else hidden_dim, hidden_dim, hidden_dim, mlp_layers, 0, 'none', self.activation, True, False))
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class SIRModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            SIRConv(1 if i == 0 else hidden_dim, hidden_dim, hidden_dim, self.activation, agg_type='max') 
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class PNAModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            dgl.nn.PNAConv(1 if i == 0 else hidden_dim, hidden_dim, ['sum', 'max', 'std'], ['identity'], 1, residual=False)
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class EGCSModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            EGConv(1 if i == 0 else hidden_dim, hidden_dim, ['symnorm'], num_heads=1, num_bases=4, add_self_loops=False)
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
...
class EGCMModel(nn.Module):
    def __init__(self, hidden_dim, output_dim=1, num_layers=1, dropout=0, **kwargs):
        ...
        self.convs = nn.ModuleList([
            EGConv(1 if i == 0 else hidden_dim, hidden_dim, ['sum', 'max', 'std'], num_heads=1, num_bases=4, add_self_loops=False)
            for i in range(num_layers)
        ])
        self.drop = nn.Dropout(dropout)
        self.classifier = nn.Linear(hidden_dim, output_dim, bias=False)
