...
def kd_loss_fn(student_logits, teacher_logits, model, args):
    return (args.kd_temp ** 2) * F.kl_div(F.log_softmax(student_logits / args.kd_temp, dim=1), 
                                          F.softmax(teacher_logits / args.kd_temp, dim=1), reduction='batchmean')
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--nlayers', type=int, default=3, help='number of graph convolution layers')
    ...
    for i in range(args.nruns):
        ...
        model = Model[args.model](_input_dim, args.nhidden, output_dim, args.nlayers, args.input_dropout, args.edge_dropout, args.dropout, args.norm,
                                  ...
                                  feat_dropout=args.feat_dropout, agg_type=args.agg_type, neigh_agg_type=args.neigh_agg_type, 
...
# SINC-GCN (100k)
# Namespace(cpu=False, gpu=0, seed=0, model='SINC', nhidden=75, nlayers=4, input_dropout=0, edge_dropout=0, dropout=0.3, norm='bn', readout_layers=1, readout_dropout=0, jumping_knowledge=False, residual=True, resid_layers=0, resid_dropout=0, feat_dropout=0.1, agg_type='mean', neigh_agg_type='sym', nheads=1, attn_dropout=0, add_self_loop=True, add_reverse_edge=True, use_xrt_emb=False, use_labels=False, label_iters=0, mask_rate=1, epochs=1000, lr=0.01, wd=0.001, factor=0.5, patience=50, min_lr=0, kd_mode='teacher', kd_alpha=0.5, kd_temp=1, flag=False, m=5, train_step_size=1e-05, untrain_step_size=1e-05, nruns=5, log_every=20, save_pred=False)
# Runned 5 times
# Val accuracy: [0.7371723883351791, 0.7375750864122957, 0.7381791335279707, 0.7394879022786, 0.7338165710258734]
# Test accuracy: [0.727568257103471, 0.726107441927453, 0.7278151554430796, 0.7255519206633335, 0.7258605435878444]
# Average val accuracy: 0.737246 ± 0.001885
# Average test accuracy: 0.726581 ± 0.000927
